...
WPROB_DIV = 173.72
TUNABLE = ["mat_mg", "mat_eg", "mob", "kdw", "kdc", "ksw", "bp_mg", "bp_eg",
...
ANCHOR = dict(
    ...
    kdc=np.array([0.4]), ksw=np.array([4.0, 8.0]), bp_mg=np.array([20.]), bp_eg=np.array([20.]),
...
def eval_cp(F, W, pst_mg, pst_eg):
    ...
    mg = mg + F["mob"] @ W["mob"]
    ub = torch.clamp(F["kd"][:, 0:4] @ W["kdw"]
                     + F["ks"][:, 0] * W["ksw"][0] + F["ks"][:, 1] * W["ksw"][1], max=KD_CAP)
    uw = torch.clamp(F["kd"][:, 4:8] @ W["kdw"]
                     + F["ks"][:, 2] * W["ksw"][0] + F["ks"][:, 3] * W["ksw"][1], max=KD_CAP)
